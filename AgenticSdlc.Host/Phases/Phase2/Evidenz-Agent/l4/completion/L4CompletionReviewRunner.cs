...
public static class L4CompletionReviewRunner
{
    ...
    public static async Task<int> RunAsync(string[] args, HostSettings settings, string repoRoot)
    {
        ...
        async Task Persist() => await File.WriteAllTextAsync(
            decisionsPath,
            JsonSerializer.Serialize(L4CompletionReviewAdapter.Apply(runId, session, proposals), Json)).ConfigureAwait(false);

        var options = new ReviewServerOptions
        {
            Session = session,
            RecomputeResolved = L4CompletionReviewAdapter.Resolved,
        ...
        Console.WriteLine($"[l4-completion-review] {result.Outcome} - {session.ResolvedCount()}/{session.Items.Count} entschieden -> human-decisions.json");
