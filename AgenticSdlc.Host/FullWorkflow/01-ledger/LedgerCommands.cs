...
public static class LedgerCommands
{
    public static void Register(IDictionary<string, AgenticSdlc.Host.CommandHandler> map)
    {
        ...
        map["ledger-build"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerBuildRunner.RunAsync(args, settings, repoRoot);
        ...
        map["ledger-build-units"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerBuildUnitsRunner.RunAsync(args, settings, repoRoot);
        ...
        map["ledger-reference-template"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerReferenceTemplateRunner.RunAsync(args, settings, repoRoot);

        // Adjudikation Schritt 1 (deterministisch, kein LLM): review_required + Misses -> Adjudikations-Queue.
        map["ledger-adjudicate"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerAdjudicateRunner.RunPrepareAsync(args, repoRoot);

        // Adjudikation apply: ausgefüllte Queue -> adjudicated-ledger + consumable + AdjudicationCompletenessGate.
        map["ledger-adjudicate-apply"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerAdjudicateRunner.RunApplyAsync(args, repoRoot);

        // Adjudikation Interactive/Re-Launch (§9): lokale Review-UI über der queue.json (Autosave), Finish -> apply.
        map["ledger-adjudicate-ui"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerAdjudicateUiRunner.RunAsync(args, repoRoot);
        ...
        map["ledger-adjudicate-refine"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerAdjudicateRefineRunner.RunAsync(args, settings, repoRoot);

        // L3 realer Test: bestehenden Ledger mit dem FacetValidator prüfen (evidence- oder transcript-Kontext).
        map["ledger-validate"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerValidateRunner.RunAsync(args, settings, repoRoot);

        // L3-B: Selective-Metrics für den FacetValidator gegen die autor-bestätigte Fixture (correct vs perturbed).
        map["facet-validation-eval"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.FacetValidationEvalRunner.RunAsync(args, settings, repoRoot);

        // Ledger-Referenz-Recall (DETERMINISTISCH, kein LLM): Segment-Overlap-Screen, gratis. Misses verlässlich.
        map["ledger-reference-recall-fast"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerReferenceRecallFastRunner.RunAsync(args, settings, repoRoot);

        // Ledger-Referenz-Recall: misst, ob ein Auto-Ledger die Claims einer Referenz findet (Completeness/(B)-Frage).
        map["ledger-reference-recall"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.Ledger.LedgerReferenceRecallRunner.RunAsync(args, settings, repoRoot);
    }
}
