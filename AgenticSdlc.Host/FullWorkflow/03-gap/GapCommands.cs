...
public static class GapCommands
{
    public static void Register(IDictionary<string, AgenticSdlc.Host.CommandHandler> map)
    {
        ...
        map["l3"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.L3.L3Runner.RunAsync(args, settings, repoRoot);

        // L3 Apply-Phase (Workflow 2): menschliche Entscheidungen (accept/edit/reject) deterministisch anwenden → Promotion + Provenienz.
        map["l3-apply"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.L3.L3ApplyRunner.RunAsync(args, settings, repoRoot);

        // L3 Reflect-Sub-Workflow (NEEDS_REVISION): Kandidaten per Feedback überarbeiten (Self-Refine) → neu klassifizieren, bounded.
        map["l3-revise"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.L3.L3ReviseRunner.RunAsync(args, settings, repoRoot);

        // L3 Human-Review (config l3.reviewMode: file|interactive): Review-UI → human-decisions.json (von apply/revise konsumiert).
        map["l3-review"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.L3.L3ReviewRunner.RunAsync(args, settings, repoRoot);
    }
}
