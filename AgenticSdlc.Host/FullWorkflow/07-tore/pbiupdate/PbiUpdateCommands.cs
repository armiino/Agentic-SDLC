...
public static class PbiUpdateCommands
{
    public static void Register(IDictionary<string, AgenticSdlc.Host.CommandHandler> map)
    {
        ...
        map["pbi-update"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.PbiUpdate.PbiUpdateRunner.RunAsync(args, settings, repoRoot);
        ...
        map["pbi-update-hitl"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.PbiUpdate.PbiUpdateHitlRunner.RunAsync(args, settings, repoRoot);

        map["pbi-update-review"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.PbiUpdate.PbiUpdateReviewRunner.RunAsync(args, settings, repoRoot);

        map["pbi-update-apply"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.PbiUpdate.PbiUpdateApplyRunner.RunAsync(args, repoRoot);
    }
}
