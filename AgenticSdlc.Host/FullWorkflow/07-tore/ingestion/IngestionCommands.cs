...
public static class IngestionCommands
{
    public static void Register(IDictionary<string, AgenticSdlc.Host.CommandHandler> map)
    {
        ...
        map["ingest-requirements"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.Core.IngestionRequirementsRunner.RunAsync(args, settings, repoRoot);
        ...
        map["ingest-requirements-hitl"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.Core.IngestionHitlRunner.RunAsync(args, settings, repoRoot);

        map["ingest-review"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.Core.IngestionReviewRunner.RunAsync(args, settings, repoRoot);

        // Deterministischer Apply: akzeptierte Operationen in den Core (Upsert-by-Identity) + Delta + affected-view.
        map["ingest-apply"] = (args, settings, repoRoot) => AgenticSdlc.Host.Phases.Phase2.EvidenzAgent.Core.IngestionApplyRunner.RunAsync(args, repoRoot);
    }
}
